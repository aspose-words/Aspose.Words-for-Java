...
public class ExFont extends ApiExampleBase
{
    ...
    public void fontInfoCollection() throws Exception
    {
        ...
        doc.save(getArtifactsDir() + "Document.docx");
        //ExEnd
    }
    ...
    public void getAllFonts() throws Exception
    {
        ...
        msConsole.writeLine("Font Count: " + fontNames.size());
        //ExEnd

        // Verify the font count is correct.
        msAssert.areEqual(2, fontNames.size());
    }

    @Test
    public void receiveFontSubstitutionNotification() throws Exception
    {
        ...
        msAssert.greater(callback.mFontWarnings.getCount(), 0);
        Assert.assertTrue(callback.mFontWarnings.get(0).getWarningType() == WarningType.FONT_SUBSTITUTION);
        Assert.assertTrue(callback.mFontWarnings.get(0).getDescription()
            .equals(
    ...
    public void fontSubstitutionWarnings() throws Exception
    {
        ...
        Assert.assertTrue(callback.mFontWarnings.get(1).getDescription()
            .equals(
    ...
    public void fontSubstitutionWarningsClosestMatch() throws Exception
    {
        ...
        doc.save(getArtifactsDir() + "Font.DisapearingBulletPoints.pdf");
    ...
    public void removeHiddenContentFromDocument() throws Exception
    {
        ...
        msAssert.areEqual(13, doc.getChildNodes(NodeType.PARAGRAPH, true).getCount()); //ExSkip
    ...
    /// </summary>
    private static class RemoveHiddenContentVisitor extends DocumentVisitor
    {
        /// <summary>
        ...
        {
            if (node instanceof Inline currentNode)
            {
            ...
            else if (node instanceof ShapeBase shape)
            {
                // Node is a shape or groupshape.
                return shape.Font.Hidden;
            }
            else if (node instanceof InlineStory inlineStory)
            {
    ...
    public void getFontInfoFromFile() throws Exception 
    {
        ...
        while (fontCollectionEnumerator.hasNext())
        {
            ...
            if (fontInfo != null)
            {
                msConsole.writeLine("Font name: " + fontInfo.getName());
                msConsole.writeLine("Alt name: " + fontInfo.getAltName()); // Alt names are usually blank
                msConsole.writeLine("\t- Family: " + fontInfo.getFamily());
                msConsole.writeLine("\t- " + (fontInfo.isTrueType() ? "Is TrueType" : "Is not TrueType"));
                msConsole.writeLine("\t- Pitch: " + fontInfo.getPitch());
                msConsole.writeLine("\t- Charset: " + fontInfo.getCharset());
                msConsole.writeLine("\t- Panose:");
                msConsole.writeLine("\t\tFamily Kind: " + (fontInfo.getPanose()[0] & 0xFF));
                msConsole.writeLine("\t\tSerif Style: " + (fontInfo.getPanose()[1] & 0xFF));
                msConsole.writeLine("\t\tWeight: " + (fontInfo.getPanose()[2] & 0xFF));
                msConsole.writeLine("\t\tProportion: " + (fontInfo.getPanose()[3] & 0xFF));
                msConsole.writeLine("\t\tContrast: " + (fontInfo.getPanose()[4] & 0xFF));
                msConsole.writeLine("\t\tStroke Variation: " + (fontInfo.getPanose()[5] & 0xFF));
                msConsole.writeLine("\t\tArm Style: " + (fontInfo.getPanose()[6] & 0xFF));
                msConsole.writeLine("\t\tLetterform: " + (fontInfo.getPanose()[7] & 0xFF));
                msConsole.writeLine("\t\tMidline: " + (fontInfo.getPanose()[8] & 0xFF));
                msConsole.writeLine("\t\tX-Height: " + (fontInfo.getPanose()[9] & 0xFF));
            }
        }
        //ExEnd
    ...
    public void fontSourceFolder() throws Exception
    {
        ...
        msAssert.areEqual(getMyDir() + "MyFonts", folderFontSource.getFolderPath());
    ...
    public void fontSourceSystem() throws Exception
    {
        ...
        if (isWindows)
        {
            ...
            Assert.AreEqual(fontsPath.toLowerCase(), SystemFontSource.getSystemFontFolders().FirstOrDefault()?.ToLower());
        }

        for (String systemFontFolder : SystemFontSource.getSystemFontFolders())
        {
            msConsole.writeLine(systemFontFolder);
        }
    ...
    public void loadFontFallbackSettingsFromStream() throws Exception
    {
        ...
        // By default fallback settings are initialized with predefined settings which mimics the Microsoft Word fallback.
        FileStream fontFallbackStream = new FileStream(getMyDir() + "Fallback.xml", FileMode.OPEN);
        try /*JAVA: was using*/
        {
            FontSettings fontSettings = new FontSettings();
    ...
    public void tableSubstitutionRuleCustom() throws Exception
    {
        ...
        FileStream fileStream = new FileStream(getMyDir() + "Font.TableSubstitutionRule.Custom.xml", FileMode.OPEN);
        try /*JAVA: was using*/
        {
