...
public class ExLists extends ApiExampleBase {
    ...
    public void listDocument() throws Exception {
        ...
        System.out.println("List is the same by ListId: " + (lists.getListByListId(1) == list));
        //ExEnd

        // Verify these properties
        Assert.assertEquals(lists.getDocument(), doc);
        Assert.assertEquals(list.getDocument(), doc);
        Assert.assertEquals(lists.getCount(), 1);
        Assert.assertEquals(lists.get(0), list);
        Assert.assertEquals(list.getListId(), 1);
        Assert.assertEquals(lists.getListByListId(1), list);
    }
