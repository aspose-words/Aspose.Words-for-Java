...
public class ExTable extends ApiExampleBase {
    ...
    public void borders() throws Exception {
        ...
        Assert.assertThrows(AssertionError.class, () -> Assert.assertEquals(0, finalTopBorder.getColor().getRGB())); //ExSkip
	...
	public static Object[][] removeParagraphTextAndMarkDataProvider() {
		return new Object[][]
		{
			{true},
			{false},
    ...
    public void allowBreakAcrossPages(boolean allowBreakAcrossPages) throws Exception {
        ...
        Assert.assertEquals(3, IterableUtils.countMatches(table.getRows(), s -> s.getRowFormat().getAllowBreakAcrossPages() == allowBreakAcrossPages));
    }

    @DataProvider(name = "allowBreakAcrossPagesDataProvider")
    public static Object[][] allowBreakAcrossPagesDataProvider() {
        return new Object[][]
                {
                        {false},
                        {true},
    ...
    public static Object[][] allowAutoFitOnTableDataProvider() {
        return new Object[][]
                {
                        {false},
                        {true},
    ...
    public static Object[][] allowCellSpacingDataProvider() {
        return new Object[][]
                {
                        {false},
                        {true},
    ...
    public void tableStyleCreation() throws Exception {
        ...
        Assert.assertEquals(6, IterableUtils.countMatches(table.getFirstRow().getRowFormat().getBorders(), s -> s.getColor().getRGB() == Color.BLACK.getRGB()));
    ...
    private void convertTable(Table table)
    {
        ...
        for (Row row : table.getRows())
        {
            for (Cell cell : row.getCells())
            {
                ...
                for (Paragraph cellPara : cell.getParagraphs())
                ...
                for (Paragraph paragraph : paragraphs)
                {
                    ...
                    else if (currentPara.getNodeType() == NodeType.PARAGRAPH)
                    {
                        ...
                        if (!cell.isFirstCell())
                        {
                            ...
                            if (separator.equals(ControlChar.TAB))
                            {
        ...
        for (Row row : table.getRows())
        {
            ...
            while (cell != null)
            {
                ...
                if (cell.getCellFormat().getHorizontalMerge() == CellMerge.FIRST)
                {
                    ...
                    cell = calculateColSpan(cell, referenceToColSpan);
                    colSpan = referenceToColSpan.get();
                }
                else
                    cell = cell.getNextCell();

                System.out.println(MessageFormat.format("RowIndex = {0}\t ColSpan = {1}\t RowSpan = {2}", rowIndex, colSpan, rowSpan));
            }
        }
    }
    ...
    private Cell calculateColSpan(Cell cell, RefInt colSpan)
    {
        colSpan.set(1);
